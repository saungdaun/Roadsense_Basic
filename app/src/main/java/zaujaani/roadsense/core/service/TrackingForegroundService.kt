    ...
    import zaujaani.roadsense.core.constants.SurveyConstants // IMPORT DITAMBAHKAN
    ...
    class TrackingForegroundService : Service() {
        ...
        private val NOTIFICATION_THROTTLE_MS = 3000L//<<---Private property name 'NOTIFICATION_THROTTLE_MS' should not contain underscores in the middle or the end
            ...
            serviceScope.launch {
                bluetoothGateway.connectionState.collectLatest { state ->
                    when (state) {
                        ...
                        // ❌ Hapus bagian else karena redundant
                    }
                }
            }
        }
        ...
        private fun buildQualityFlags(
            ...
            sensorData.batteryVoltage
                ?.takeIf { it < SurveyConstants.BATTERY_LOW_VOLTAGE }
                ?.let { voltage ->

                    val flag = if (voltage < SurveyConstants.BATTERY_CRITICAL_VOLTAGE) {
                        QualityFlag.BATTERY_CRITICAL
                    } else {
                        QualityFlag.BATTERY_LOW
                    }
